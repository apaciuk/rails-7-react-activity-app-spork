class TaskActivitiesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @task_activity.save
        format.html { redirect_to task_activity_url(@task_activity), notice: 'Task activity was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @task_activity.update(task_activity_params)
        format.html { redirect_to task_activity_url(@task_activity), notice: 'Task activity was successfully updated.' }
  ...
  def task_activity_params
    params.require(:task_activity).permit(:title, :description, :completed)
  end
end
